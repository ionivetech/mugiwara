...
10. After each wave, ensure the mission decision log (`.mugiwara/logs/YYYY-MM-DD-<mission>.md`) is updated — every wave performed recorded with outcome and duration. Each heal cycle is a `## Wave 8 — healing` section; savepoint counts those sections for `heal_cycle`, so an unlogged heal wave reads as no cycle.
...
14. At sub-mission closure, if mission belongs to a team initiative, set that sub-mission's status cell to `done` in the initiative plan doc. (`scripts/initiative.ts` automates this, but it is repo-development-only — never installed, and `mugiwara run` takes `.sh` scripts only.)
