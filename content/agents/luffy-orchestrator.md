...
13. Terminal per mode (per `mugiwara-mode`): save-point commit → push the mission branch with plain `git push -u origin <branch>` (per the config `branch` key) → write the PR verdict per `mugiwara-pr` (with a copy-paste PR description). guided/semi: hand branch + verdict to the user, who opens the PR. auto: delegate to the `mugiwara-pr` auto path (forge-detect → `gh`/`glab`/Bitbucket REST → URL fallback) and report the PR URL. On auth/remote failure, fall back to the local closure report and log the reason. The crew never merges, never deploys, never auto-reacts to review comments or CI in any mode.
