...
Terminal step (every mode): save-point commit → push the mission branch (per the config `branch` key, default `feature/{type}-{issue}-{slug}`) → `gh pr create` with the config `pr` key (default `ready`, never `--draft`) → write `.mugiwara/results/YYYY-MM-DD-<mission>-pr-verdict.md` per the `mugiwara-pr` format and post ONE comment + check-run → hand the PR link to the user. The crew never merges, never deploys. On push/PR failure (no auth / no remote), fall back to the local closure report and log the reason. Never auto-react to review comments or CI in any mode.
