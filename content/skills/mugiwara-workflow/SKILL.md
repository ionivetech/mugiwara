...
**Auto/semi continuation.** In `auto`/`semi` a handoff ends a segment, never the turn: the next role starts at once with its banner, `savepoint --flow N` recorded behind it. Only a genuine blocker or `heal_halt` yields to the user. In `guided` the turn ends at every handoff for the user's GO. `semi` differs from `auto` only at the Flow 0 roster, ambiguity rounds, and commits without `auto_commit=on` — never at stage continuation.
