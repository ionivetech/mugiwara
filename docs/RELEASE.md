...
| `node scripts/validate-content.mjs` | `✓ content valid: 21 skills, 14 agents` |
...
- `content/` — **35 files** (14 agents + 21 skills)
...
The package currently has 14 agents + 21 skills (`content/`), 8 targets (`src/targets/`), and is at `0.1.0` — pre-1.0, so per semver `0.x` behaves loosely; treat "feature → minor, fix → patch" as the default until `1.0.0`.
