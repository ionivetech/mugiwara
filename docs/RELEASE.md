...
| `node scripts/validate-content.mjs` | `✓ content valid: 12 skills, 10 agents` |
...
- `content/` — **22 files** (10 agents + 12 skills)
...
The package currently has 10 agents + 12 skills (`content/`), 8 targets (`src/targets/`), and is at `0.1.0` — pre-1.0, so per semver `0.x` behaves loosely; treat "feature → minor, fix → patch" as the default until `1.0.0`.
