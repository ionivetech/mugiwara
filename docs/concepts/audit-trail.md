...
A mission that ends with "trust me" leaves nothing to review. Claims without evidence cannot be checked. This page answers "what is recorded" with one example, then the artifact table. Every verdict points at files a reviewer can open.
...
Reviewer order: report first, the [gate](features.md#quality-and-coverage-gates) verdict second, audit spot-check third, findings count fourth, [provenance](provenance.md) fifth, raw state numbers last. Commit the trail: plan, spec, decisions, blockers, review, security, flows, and report stay versioned. State JSON stays ignored and recomputed. A trail that vanishes at merge is no trail.
