...
Luffy sizes the diff to a full lane, Usopp sharpens the vague edges, Nami plans [waves](#plans-with-acceptance-checks) with acceptance checks, Zoro implements with evidence per task, Chopper re-runs each check, Sanji and Franky gate, Robin and Jinbe review, Brook heals failures, Luffy closes with a report plus a ready PR summary.
...
What: guided asks before each [flow stage](workflow.md), semi runs from an approved plan, auto runs triage to closure and pauses only on a genuine blocker. A flip applies from the next stage, never mid-stage.
