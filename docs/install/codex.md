...
Ask what crew members are available; a correct install answers with the roster. `mugiwara list --check` reports missing and stale files as a health pass. Update with the update command naming project, target, and confirmation. Codex is tier 3, so the main thread embodies each persona from markdown and reads the full body on demand. A host-native `codex plugin add` path is untested on this host; the CLI path above is the verified one.

The crew-wide state router (`status`, `continue`, `cost`, `archive`, `clean`, `handoff`, `sign`, `lesson`, `migrate`) runs through `mugiwara ...` or `npx -y @ionivetech/mugiwara@latest ...`, and bare `archive`/`handoff`/`sign` list missions to pick (exit 2). Only Claude Code and opencode add the `/mugiwara` slash-command wrapper.

## Clean uninstall

`mugiwara uninstall [--global]` removes exactly what the manifest recorded (check with `mugiwara list --check`). Remove by hand what the manifest never owned: `.codex/mugiwara/` rule files, `.mugiwara/refs/` rule bodies, and the pointer line in `AGENTS.md` if your file pre-existed the install. `.mugiwara/config` and the `.gitignore` block are yours and stay unless you remove them. Verify: the roster question no longer lists the crew.
