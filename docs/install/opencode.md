...
Switch autonomy at runtime by saying `mugiwara mode guided`, `mugiwara mode semi`, or `mugiwara mode auto` in session (no slash, no CLI flag); bare `/mugiwara` shows the current mode. Verify through the router or the roster question. Upgrade by removing `~/.cache/opencode/packages/@ionivetech/`, restarting, and reinstalling; package updates never touch OpenCode's pinned copy. Uninstall below; removing the plugin line alone leaves the pinned copy behind. Action mapping: todos via todowrite, subagents via task, skills via skill, files via read, edit, write, shell via bash, search via grep and glob, URLs via webfetch. Report persistent failures at the repository issue tracker.
## Clean uninstall
