...
| Rule | Mechanism |
|------|-----------|
| Lane sizing | `mugiwara run lane.sh` computes lane from `git diff --name-only` |
| State persistence | `mugiwara savepoint` writes the mission state at every wave boundary |
| Evidence capture | `mugiwara run evidence.sh <mission> <label> -- <cmd>` writes stdout/stderr to `.mugiwara/results/<mission>/<label>-<hash>.log` |
...
| Wave boundaries | Every wave opens with the agent-colored banner (`===== <emoji> WAVE N — <CREW> (ROLE) =====` — ANSI-wrapped in terminals, plain in markdown UIs), closes with checkpoint report. |
