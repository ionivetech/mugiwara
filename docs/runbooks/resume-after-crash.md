...
The session died and the plan is on disk, not in your context. Rebuild the picture from state instead of starting over.
...
2. Several missions: list, then pick.
   ...
   ```
   ```
   2 missions in flight:

     MISSION  MEMBER           FLOW  TASKS  LANE    MODE
     demo     sophia-martinez  1     0/0    direct  guided
     m        jane-doe         1     0/0    direct  guided

   Pick one: mugiwara continue <mission> [member]
   ```
3. Several handles on one mission with a roster produce a numbered picker
...
4. Handing over to someone else: write the report they can act on.
   ...
   Generated 2026-09-09T05:47:06.327Z by `mugiwara handoff`.
   … [trimmed]
   | Mission | flow 1, tasks 0/0, lane direct, mode guided |
   … [trimmed]
