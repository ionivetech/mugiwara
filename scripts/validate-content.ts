...
if (process.argv.includes('--check-invariants')) {
  ...
  const CONCEPTS: Array<{ id: string; re: RegExp; anchor: string }> = [
    ...
    { id: 'INV-evidence', re: /evidence|re-run|re run|claim|never validate|doubt|fresh (context|agent)|adversarial|no pass|canary|looks fine|never 0|never restored|never exercised/i, anchor: 'INV-evidence' },
    ...
    { id: 'INV-plan-discipline', re: /zero-question|unverified path|\btbd\b|plan above or below|40-file|task index|stranger must|plan.*hole|regression|correctness.*break|never.*plan|never appended|2000\+ lines|re-index|never renumber|bare ids|bare file/i, anchor: 'INV-plan-discipline' },
    ...
    { id: 'INV-git-hygiene', re: /commit|git add|revert|broken tree|staging|micro-commit|never.*tree|force-add|gitignore|never rewrite|amend forward|git history/i, anchor: 'INV-git-hygiene' },
    { id: 'INV-conduct', re: /ego|yes-man|interrogat|trade-off|assume silently|batched question|sparring|recommendation|reconsider|pushes back|silently defer|verdict only|never recommends|reports but never|creative way out|bare "no"|middle option/i, anchor: 'INV-conduct' },
